...
## 콘텐츠는 두 종류다
1. **구조화 콘텐츠 (콘텐츠 레이어, zod 검증됨)** — `src/content/*.ts` + `content/projects/*.mdx` + `src/config/site.ts`. 값만 바꾸면 화면 자동 반영. 형식 틀리면 **빌드 실패**(아래 규칙 참고).
2. **하드코딩 카피 (컴포넌트 안 JSX)** — `src/components/About.tsx`·`Contact.tsx`의 문단. 콘텐츠 레이어가 아니라 컴포넌트 파일을 직접 편집. (`{/* 사용자가 ... 교체 */}` 주석 표시되어 있음.)

---

## 채울 위치 체크리스트

### ☐ 1. 사이트 메타·연락처 — `src/config/site.ts`
| 필드 | 현재(목업) | 채울 것 |
|---|---|---|
...
### ☐ 2. 프로필 / Hero / Career Snapshot — `src/content/profile.ts`
| 필드 | 현재(목업) | 채울 것 |
|---|---|---|
...
### ☐ 3. 경력 타임라인 — `src/content/experience.ts`
...
- `src/components/About.tsx` — 3문단(소개). 전부 목업 카피.
- `src/components/Contact.tsx` — 안내 1문단(`새로운 기회나 협업…`). 목업 카피. (CTA 링크는 `profile`에서 자동.)
...
스키마는 `src/content/schema.ts`(zod). 핵심:
...
## 바꾸지 말 것 (구조)
- `src/content/schema.ts`(스키마)·컴포넌트 마크업·레이아웃·토큰·인터랙션. **값만** 바꾼다. 새 필드가 필요하면 스키마부터 합의.
