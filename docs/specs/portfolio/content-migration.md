...
| 저장소 | 역할 | 비고 |
|---|---|---|
| **vd-front** | VDOC PRO PC (데스크톱, Electron) | **소스코드 + (선택) verbatim raw** — 코드 `src/features/custom`, 원문 `docs/portfolio/<회사>/raw/`(⛔push금지). *작성·발행 포함 모든 산출물은 jio.dev* (→ §4). |
...
- **소스·원문 보관**: `vd-front/docs/portfolio/` (브랜치 `docs/pp`, ⛔push금지) — 소스코드 + (선택) verbatim raw. ⚠️ **vd-front `docs/pp`는 임시 브랜치라 지속 문서작업 불가** → *원자료·인터뷰·작성·발행 모든 산출물은 jio.dev에서 한다* (→ §4 위치 표).
...
| (선택) verbatim raw | 인터뷰 원문·정확 인용(파일:라인·커밋·티켓) | **vd-front** `docs/portfolio/<회사>/raw/` | commit-only ⛔**push 금지** | 원문 OK |
...
- **vd-front의 역할 = 소스코드 + (선택) verbatim raw 보관만**. ⛔ `docs/portfolio/`는 절대 push 금지(개인정보 포함, `git commit`만). 작성된 글은 여기 두지 않는다.
